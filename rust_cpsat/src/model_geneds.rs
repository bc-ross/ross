...
pub fn add_gened_constraints<'a>(ctx: &mut ModelBuilderContext<'a>) {
    // Print Foundation sets, requirements, and pairwise intersections for debugging
    eprintln!("[FOUNDATION DIAG] Foundations: {}", num_foundations);
    for (f, (set, required)) in foundation_reqs.iter().enumerate() {
        let codes: Vec<_> = set.iter().map(|&idx| &courses[idx].code).collect();
        eprintln!("[FOUNDATION DIAG] Foundation {}: required = {}, eligible = {:?}", f, required, codes);
    }
    for i in 0..num_foundations {
        for j in (i+1)..num_foundations {
            ...
            let union: HashSet<_> = set_i.union(&set_j).copied().collect();
            let req_i = foundation_reqs[i].1;
            let req_j = foundation_reqs[j].1;
            let min_overlap = (req_i + req_j).saturating_sub(union.len() as i64);
            let codes: Vec<_> = intersection.iter().map(|&idx| &courses[idx].code).collect();
            eprintln!("[FOUNDATION DIAG] Pair ({}, {}): intersection = {}, union = {}, reqs = ({}, {}), min_overlap = {}, codes = {:?}", i, j, intersection.len(), union.len(), req_i, req_j, min_overlap, codes);
        }
    }
    ...
    // For every pair of Foundations, require that the number of scheduled courses in the intersection is at most the minimal overlap required
    for i in 0..num_foundations {
        for j in (i+1)..num_foundations {
            let set_i: HashSet<_> = foundation_reqs[i].0.iter().copied().collect();
            let set_j: HashSet<_> = foundation_reqs[j].0.iter().copied().collect();
            ...
            let req_j = foundation_reqs[j].1;
            let min_overlap = (req_i + req_j).saturating_sub(union.len() as i64);
            eprintln!("[FOUNDATION DIAG] Pair ({}, {}): intersection = {}, union = {}, reqs = ({}, {}), min_overlap = {}", i, j, intersection.len(), union.len(), req_i, req_j, min_overlap);
            if !intersection.is_empty() {
                let mut sum = LinearExpr::from(0);
                for &idx in &intersection {
                    sum = sum + course_in_schedule(idx);
                }
                // Allow minimal overlap if required
                model.add_le(sum, LinearExpr::from(min_overlap));
            }
        }
    }
